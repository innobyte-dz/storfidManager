 ...
 * {@link AddFieldFragment.OnFragmentInteractionListener} interface
...
public class AddFieldFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View v =  inflater.inflate(R.layout.fragment_add_field, container, false);

        btn_save  = (ImageButton) v.findViewById(R.id.btn_save);
        nom_field = (MyEditText) v. findViewById(R.id.nom_field);

        btn_save.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                try{
                String name = nom_field.getText().toString();
                FieldList f = new FieldList();
                f.setFieldName(name);
                f.save();
                AndroidUtility.alert(getContext(),"Attribut enregistré");
                    nom_field.setText("");
            }catch (Exception s){
                    AndroidUtility.alert(getContext(),"Erreur, "+s.getMessage());
                }
            }
        });
